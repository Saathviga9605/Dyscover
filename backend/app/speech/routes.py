...
from app.l10n import locale_for
...
def create_session(payload: SpeechSessionCreate, db: Session = Depends(get_db)) -> SpeechSession:
	...
	locale = payload.locale or locale_for(payload.language)
	speech_session = SpeechSession(assessment_id=payload.session_id, trial_id=payload.trial_id, task_id=payload.task.task_id, expected_text=payload.task.expected_text, language=payload.language, locale=locale, provider=payload.provider, provider_version=payload.provider_version, audio_available=payload.audio_available, duration_ms=payload.duration_ms)
	db.add(speech_session)
	db.commit()
	db.refresh(speech_session)
	return {"id": speech_session.id, "session_id": speech_session.assessment_id, "trial_id": speech_session.trial_id, "task": payload.task, "language": speech_session.language, "locale": speech_session.locale, "provider": speech_session.provider, "provider_version": speech_session.provider_version, "audio_available": speech_session.audio_available, "duration_ms": speech_session.duration_ms, "created_at": speech_session.created_at}
...
def get_session(speech_session_id: UUID, db: Session = Depends(get_db)) -> SpeechSession:
	...
	return {"id": speech_session.id, "session_id": speech_session.assessment_id, "trial_id": speech_session.trial_id, "task": {"task_id": speech_session.task_id, "expected_text": speech_session.expected_text, "language": speech_session.language}, "language": speech_session.language, "locale": speech_session.locale, "provider": speech_session.provider, "provider_version": speech_session.provider_version, "audio_available": speech_session.audio_available, "duration_ms": speech_session.duration_ms, "created_at": speech_session.created_at}
